...
void RxStatistics::save_channel_characteristics(std::complex<int8_t> _dc_offset,
                                                float _noise_power) {
  std::lock_guard<std::mutex> lock(mutex);
...
std::string RxStatistics::to_string() {
  ...
  // there is a minor trickery here: noise power is calculated from sample
  // variance (sigma-squared), while ADC_FULL_SCALE represents a voltage. As
  // such, rssi = 10*log(Psig/Pmax)
  //      = 10*log(Psig) - 10*log(Pmax)
  //      = 10*log(Psig) - 20*log(Vmax)
  float noise_floor =
      10.0f * std::log10f(noise_power) - 20.0 * std::log10f(ADC_FULL_SCALE);

  std::string temp;
  std::format_to(std::back_inserter(temp), "{} {} {} {}", noise_floor,
                 std::abs(dc_offset), frames_received, frames_processed);
  return temp;
}