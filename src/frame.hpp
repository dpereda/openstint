...
class FrameDetector {
  ...
  // preamble matching
  static inline const Preamble<uint16_t> p_openstint{
  ...
  float variance =
      0; // ~noise power (expected value squared after dc offset removal)
...
public:
  ...
  void read_preamble(Frame *dst, std::complex<int8_t> offset,
                     const std::complex<int8_t> *src, int end);
  void read_symbol(Frame *dst, std::complex<int8_t> offset,
...
private:
  uint32_t read_single(Frame *dst, float scale,
                       const std::complex<int8_t> offset,
                       const std::complex<int8_t> *src);
  void read_preamble0(Frame *dst, float scale, std::complex<int8_t> offset,
                      const std::complex<int8_t> *src, int end);
};
