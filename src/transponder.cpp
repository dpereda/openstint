...
static void *viterbi_decoder_k9; // for OPN (K=9)
static void *viterbi_decoder_k7; // for RC4 (K=7)
static crc_scheme crc8_scheme = LIQUID_CRC_8;

void init_transponders() {
  viterbi_decoder_k9 = create_viterbi29(32);
  viterbi_decoder_k7 = create_viterbi27(144); // 144 bits for RC4
}

int decode_openstint(const uint8_t *softbits, uint32_t *transponder_id) {
  uint8_t decoded[4];

  init_viterbi29(viterbi_decoder_k9, 0);
  update_viterbi29_blk(viterbi_decoder_k9, const_cast<uint8_t *>(softbits),
                       32 + 8); // khm...
  chainback_viterbi29(viterbi_decoder_k9, decoded, 32, 0);
...
int decode_legacy(const uint8_t *softbits, uint32_t *transponder_id) {
  // RC3 use a K=24, r=1/2 convolutional encoder with polynoms 0xEEC20F and
  // 0xEEC20D Decoding this properly with error correction must have some
  // unknown trick. However, we can do non-trivial decoding as well. Note, the
  // generating polynoms differ only in one bit (0xf=1111 vs 0xd=1101). This
  // reduces the complexity to: bit0 = parity(SHREG & 0xEEC20C) ^ SHREG[1] ^
  // SHREG[0] bit1 = parity(SHREG & 0xEEC20C) ^            SHREG[0] As such, we
  // know: bit0 ^ bit1 = SHREG[1] SHREG[0] = bit0 ^ parity(SHREG & 0xEEC20C) ^
  // SHREG[1] SHREG[0] = bit1 ^ parity(SHREG & 0xEEC20C) we can use these for a
  // makeshift error correction
  ...
  for (int i = 0; i < 80; i += 2) {
    ...
    if (last_ok) { // no error correction for SHREG[1] is needed
      ...
      // SHREG[0]'  = b0 ^ PAR(...) ^ SHREG[1] = b0 ^ PAR(...) ^ (b0 ^ b1) =
      // PAR(...) ^ b1 = shreg0p1
  ...
  // Example: your transponder id is "1234567", or
  // "0b00010010_11010110_10000111". To get the pre-encoded message, reverse the
  // binary word and break into chunks of 3. You'll end up with 24/3=8 chunks:
  // 111 000 010 110 101 101 001 000
  // Suffix each chunk with a bit from a "status code", in my transponder, it
  // was 00000101 1110 0000 0100 1100 1010 1011 0010 0001
  ...
  *transponder_id = tid;
  ...
  // Return a bitfield: 0 = SUCCESS, 1 = Trail Non-Zero, 2 = Status Invalid, 3 =
  // Both
  int fail_reason = 0;
  if (trail != 0)
    fail_reason |= 1;
  ...
  // For debugging, we also want to see the status byte in main.cpp.
  // We'll repurpose the return value: 0 is success, negative is failure with
  // reason.
  if (fail_reason == 0)
    return 1; // Success
  return -((static_cast<int>(status) << 8) |
           fail_reason); // Return encoded status and reason
}
