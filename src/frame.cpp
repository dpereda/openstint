...
Frame::Frame(TransponderType _ttype, uint64_t _ts, float preamble_energy)
  ...
  // for normalization post-symsync, we're better off with RMS, hence this
  // transform
  symbol_magnitude =
      std::sqrtf(preamble_energy) / std::sqrtf(2.0f); // amplitude^2 to RMS
}
...
const uint8_t *Frame::bits() {
  ...
  int pos =
      preamble_pos(sof, transponder_props(transponder_type).bpsk_preamble);
  if (pos < 0) {
    ...
    if (pos < 0) { // preamble not found
      ...
      std::transform(softbits.begin(), softbits.end(), softbits.begin(),
...
float Frame::evm() const { return evm_sum / softbits.size(); }

std::ostream &operator<<(std::ostream &os, const Frame &f) {
  std::stringstream s;
  std::copy(f.softbits.begin(), f.softbits.end(),
            std::ostream_iterator<int>(s, ", "));
  return os << transponder_props(f.transponder_type).prefix
            << " T:" << f.timestamp << " RSSI:" << f.rssi()
            << " EVM:" << f.evm() << " [" << s.str() << "]";
}
...
FrameDetector::process_baseband(const std::complex<int8_t> *samples) {
  ...
  std::transform(
      samples, samples + samples_per_symbol, sb,
      [this](const std::complex<int8_t> s) { return s - this->offset; });
  std::transform(sb, sb + samples_per_symbol, mag2s,
                 [](const std::complex<int8_t> s) {
                   return std::norm(complex_cast<int16_t>(s));
                 });
  ...
  // Print debug every ~1 second (5M samples / 4 samples per call = 1.25M calls)
  if (debug_sample_count >= 1250000) {
    std::fprintf(stderr,
                 "[DEBUG] Max correlation: %.4f (threshold: %.2f), DC offset: "
                 "(%d, %d)\n",
                 max_corr_score, threshold, offset.real(), offset.imag());
    max_corr_score = 0.0f;
...
void FrameDetector::update_statistics() {
  if (n > STATS_UPDATE_THRESHOLD) {
    ...
    offset = std::complex<int8_t>(
        static_cast<int8_t>(std::round(static_cast<float>(s1.real()) / n_f)),
        static_cast<int8_t>(std::round(static_cast<float>(s1.imag()) / n_f)));
    variance = static_cast<float>(s2) /
               (n_f - 1.0f); // sample's variance (vs population variance)
...
float FrameDetector::symbol_energy() const {
  uint32_t wes[4] = {buffers[0].window_energy, buffers[1].window_energy,
                     buffers[2].window_energy, buffers[3].window_energy};
  int idx = std::distance(wes, std::max_element(wes, wes + 4)); // ~maxarg

  return static_cast<float>(buffers[idx].window_energy) / 16.0f;
}

float FrameDetector::noise_energy() const { return variance; }

std::complex<int8_t> FrameDetector::dc_offset() const { return offset; }
...
uint32_t SymbolReader::read_single(Frame *dst, float scale,
                                   const std::complex<int8_t> offset,
                                   const std::complex<int8_t> *src) {
  // convert i8 to f32
  ...
  symsync_crcf_execute(symsync, bf, samples_per_symbol, symbols,
                       &symbols_written);
  // demodulate
  if (dst) {
    for (auto i = 0u; i < symbols_written; ++i) {
      ...
      modemcf_demodulate_soft(bpsk_modem, symbols[i] * correction, &bit,
                              &soft_bit);
...
void SymbolReader::read_preamble0(Frame *dst, float scale,
                                  std::complex<int8_t> offset,
                                  const std::complex<int8_t> *src, int end) {
...
void SymbolReader::read_preamble(Frame *dst, std::complex<int8_t> offset,
                                 const std::complex<int8_t> *src, int end) {
  symsync_crcf_reset(symsync);
  symsync_crcf_set_lf_bw(symsync, 0.02f);
...
void SymbolReader::update_reserve_buffer(const std::complex<int8_t> *src,
                                         int end) {
  const int n_preserved = MAX_PREAMBLE * samples_per_symbol;
  std::memcpy(reserve_buffer, src + end - n_preserved,
              n_preserved * sizeof(std::complex<uint8_t>));
}

void SymbolReader::read_symbol(Frame *dst, std::complex<int8_t> offset,
                               const std::complex<int8_t> *src) {
  read_single(dst, dst->symbol_magnitude, offset, src);
}

bool SymbolReader::is_frame_complete(const Frame *f) {
  // we read the preamble + -1th bit to initialize differential-BPSK
  // demodulation payload, obviously the symbol-sync's filters has their own
  // delay
  return f->softbits.size() >
         (f->preamble_size + 1 + f->payload_size + 2 * filter_delay);
}