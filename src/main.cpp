...
static const uint8_t DEFAULT_LNA_GAIN =
    24; // 0-40 in steps of 8 or so; experiment
static const uint8_t DEFAULT_VGA_GAIN = 24; // 0-62
static const int DEFAULT_ZEROMQ_PORT = 5556;

static enum FrameParseMode {
  FRAME_SEEK,
  FRAME_FOUND
} frame_parse_mode = FRAME_SEEK;
static float detection_threshold = 0.70f; // Default, can be overridden with -t
static std::unique_ptr<FrameDetector> frame_detector;
...
static const uint64_t startup_ts =
...
int main(int argc, char **argv) {
  SdrBackend backend =
      SdrBackend::HackRF; // Default to HackRF for backward compatibility
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    if (arg == "-r") {
      ...
      detection_threshold = std::atof(argv[++i]);
      if (detection_threshold < 0.1f || detection_threshold > 1.0f) {
        std::cerr << "Error: Threshold must be between 0.1 and 1.0.\n";
        return 1;
      }
    } else {
      if (arg != "-h") {
      ...
      std::cerr << "Usage: " << argv[0]
                << " [-r] [-d ser_nr] [-p tcp_port] [-g <0..100>] [-l <0..40>] "
                   "[-v <0..62>] [-a] [-b] [-m]\n";
      std::cerr
          << "\t-r          default:off \tUse RTL-SDR instead of HackRF\n";
      std::cerr << "\t-d ser_nr   default:first\tserial number of the desired "
                   "device\n";
      std::cerr << "\t-p port     default:" << DEFAULT_ZEROMQ_PORT
                << "\tZeroMQ publisher port\n";
      std::cerr << "\t-g <0..100> default:50  \tUnified gain (works with both "
                   "HackRF and RTL-SDR)\n";
      std::cerr << "\t-l <0..40>  default:"
                << static_cast<int>(DEFAULT_LNA_GAIN)
                << "  \tLNA gain [HackRF only] (rf signal amplifier; valid "
                   "values: 0/8/16/24/32/40)\n";
      std::cerr << "\t-v <0..62>  default:"
                << static_cast<int>(DEFAULT_VGA_GAIN)
                << "  \tVGA gain [HackRF only] (baseband signal amplifier, "
                   "steps of 2)\n";
      std::cerr << "\t-a          default:off \tEnable preamp/LNA boost\n";
      std::cerr << "\t-b          default:off \tEnable bias-tee (+3.3 V, 50 mA "
                   "max)\n";
      std::cerr << "\t-n          default:off \tDisable RTL-SDR direct "
                   "sampling (force tuner/V4 mode)\n";
      std::cerr << "\t-m          default:off \tEnable monitor mode (print "
                   "received frames to stdout)\n";
      std::cerr
          << "\t-i          default:off \tInvert IQ spectrum (fix for some "
             "upconverters)\n";
      std::cerr
          << "\t-t <0.1..1> default:0.70\tDetection threshold (lower = more "
  ...
  // Create frame detector with user-specified threshold
  frame_detector = std::make_unique<FrameDetector>(detection_threshold);
  std::cerr << "Detection threshold: " << detection_threshold << "\n";
  ...
  auto rx_callback = [&](const std::complex<int8_t> *samples,
    ...
    for (uint32_t idx = 0; (idx + SAMPLES_PER_SYMBOL) <= sample_count;
         idx += SAMPLES_PER_SYMBOL) {
      if (frame_parse_mode == FRAME_SEEK) {
        const std::optional<TransponderType> detected =
            frame_detector->process_baseband(samples + idx);
        if (detected) {
  ...
  config.direct_sampling_enabled = direct_sampling;
  config.direct_sampling_enabled = direct_sampling;
  config.device_serial = device_serial;
  config.invert_iq = invert_iq;
  ...
  std::cout << sdr_device->get_backend_name() << " RX: freq=" << freq_hz
            << " Hz, sample_rate=" << sample_rate << " Hz\n";
  ...
  while (!do_exit && sdr_device->is_streaming()) {
    ...
    if (rx_stats.reporting_due(now)) {
      const std::string report =
          std::format("S {} {}", now, rx_stats.to_string());
      rx_stats.reset(now);
    ...
    std::vector<TimeSync> timesyncs =
        passing_detector.identify_timesyncs(500ul);
    for (const auto &time_sync : timesyncs) {
      const std::string report = std::format(
          "T {} {} {} {}", time_sync.timestamp,
          transponder_props(time_sync.transponder_type)
              .prefix, // always openstint
          time_sync.transponder_id, time_sync.transponder_timestamp);
    ...
    std::vector<Passing> passings =
        passing_detector.identify_passings(now - 250ul);
    for (const auto &passing : passings) {
      const std::string report = std::format(
          "P {} {} {} {:.2f} {} {:.2f}", passing.timestamp,
          transponder_props(passing.transponder_type).prefix,
          passing.transponder_id, passing.rssi, passing.hits, passing.evm);
